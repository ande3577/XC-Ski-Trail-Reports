...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		//webtrac account name
		//setHeaderHousehold();
		$("#search").focus(function(){
			if ($(this).val() == "keyword or event ID") {$(this).val("");}
		});
		$("#search").blur(function(){
			if ($(this).val() == "") {$(this).val("keyword or event ID");}
		});
		$(".gobutton").click(function(){
			var criteria = $("#search").val();
			window.location = "/search-results.aspx?searchCriteria=" + escape(criteria);
		});
...
<div class="news_article">
	...
	<script type="text/javascript">
		jQuery(document).ready(function ($) {
			var dynamicOffset = 20;
			if ($.browser.msie && parseInt($.browser.version) == 6 || 7) {
				dynamicOffset = 46;
			}
			...
			//initialize tooltip
			$(".jcarousel-item a").cluetip({
				width: 180,
				positionBy: 'fixed',
				topOffset: dynamicOffset,
				leftOffset: -115,
				cluetipClass: 'small'
			});
			//remove image tooltip in IE
			$(".jcarousel-item img").attr("title", "");
			var interval;
			var path;
			...
			$(".jcarousel-item a").live("click", function () {
				...
				if (className == "video") {
					$("#carousel_media").html("<div id='video'><p class='error'>Flash player 9 or higher is required to view this video.</p></div>");
					$("#carousel_bio div").hide();
					playVideo();
				} else if (className == "webcam") {
					$("#carousel_media").html("<img src='" + image + "' alt='" + alt + "' id='webcam' width='440' height='330' />");
					$("#carousel_bio div").hide();
					startInterval();
				} else if (className !== "") {
					$("#carousel_media").html("<img src='" + image + "' alt='" + alt + "' width='440' height='330' />");
					$("#carousel_bio div").hide();
					$("#carousel_bio ." + className + "").show();
				} else {
					$("#carousel_media").html("<img src='" + image + "' alt='" + alt + "' width='440' height='330' />");
					$("#carousel_bio div").hide();
				};
				...
				function startInterval() {
					interval = setInterval(reloadImage, 5000);
				};
				...
				function reloadImage() {
					...
					webcam.src = image + '?' + Math.random();
				};

				return false;
			});

			//initialize flash video
			function playVideo() {
				var flashvars = {};
				flashvars.videoPath = path;
				var params = {};
				var attributes = {};
				swfobject.embedSWF("/flash/VideoPlayer.swf", "video", "440", "330", "9.0.28", false, flashvars, params, attributes);
			}

			//hide bios on page load
			$("#carousel_bio div").hide();
			//show first media item on page load
			$(".jcarousel-item a:first").click();
		});
	</script>
...
<div class="block_box"><ul class="icon_list"><li><a href="/activities/cross-country-skiing.aspx"><img src="~/media/Activity Icons/x-country_skiing_20px.ashx" alt="cross country skiing" width="20" height="20" /><span>Cross-country Skiing</span></a></li></ul></div>
	...
	<!-- start of footer -->
	<div id="print_footer">
		<div id="print_logo"><img width="80" height="40" alt="Three Rivers Park District" src="/assets/logo_print.jpg" /></div>
